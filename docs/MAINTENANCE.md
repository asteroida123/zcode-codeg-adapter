...
依赖包的 package.json 主入口 `dist/index.js` 及其 `main()` 导出。这是当前上游已实现的模块入口，**不是上游承诺稳定的嵌入 API**。所以精确锁版本并做真实包 smoke，而不是依赖 latest 或 ^。若上游将来提供专用无副作用 server 导出，可用独立小变更切换；不能直接导入其 handlers/translator 私有路径。
...
- package.json 只改一个精确上游版本，更新 package-lock.json；不要手写 integrity/锁文件，不自动合并依赖机器人 PR。
